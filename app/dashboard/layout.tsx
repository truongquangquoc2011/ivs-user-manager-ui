  ...
  return (
    ...
    >
      <span
        ...
        style={{ color: active ? 'var(--nav-active-color)' : 'var(--nav-icon-color)' }}
      >
        {icon}
      </span>
...
function Sidebar({
  ...
  const visibleNav = NAV_ITEMS.filter(
    (item) => item.feature === 'PROFILE' || hasPermission(item.feature, 'canView')
  );
  ...
  // Khi collapsed, click vào sidebar sẽ mở ra
      ...
      >
        ...
        >
          ...
          {!collapsed && (
            <button
              ...
              title="Thu gọn"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--toggle-color)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        ...
        >
            ...
            >
              ...
              {!collapsed && (
                <div style={{ minWidth: 0, overflow: 'hidden', flex: 1 }}>
                  ...
                  >
                    {profile.groups.map((g: { name: string }) => g.name).join(', ')}
                  </p>
                </div>
              )}

              {/* Logout icon in compact user row */}
              {!collapsed && (
                <button
                  onClick={(e) => {
                    e.preventDefault();
                  ...
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLButtonElement).style.color = 'var(--logout-color)';
                    (e.currentTarget as HTMLButtonElement).style.background = 'var(--logout-hover)';
                  }}
                  onMouseLeave={(e) => {
          ...
          {collapsed && (
            ...
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--logout-color)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4M16 17l5-5-5-5M21 12H9"/>
              </svg>
              <span className="sidebar-tooltip absolute left-full ml-3 px-2.5 py-1.5 rounded-lg text-xs whitespace-nowrap shadow-lg pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-50">
                Đăng xuất
...
function DashboardInner({ children }: { children: React.ReactNode }) {
  ...
  if (loading) {
    return (
      ...
      >
        <div style={{ fontSize: '13px', color: '#aaa' }}>Đang tải...</div>
      </div>
    );
  }
